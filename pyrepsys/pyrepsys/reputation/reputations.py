...
# note: without the Weights improvement handler enabled,
# this rep strat does the same as its brother with no weights support
class BasedOnAvgDifferenceOfClaimsAndReviewsWithWeight(ReputationStrategy):
    def calculate_reputations(self, agents):
            ...
            for claim in agent.claims:
                ...
                if reviews:
                    ...
                    for r, w in zip(reviews,weights):
                        weighted_sum += r*w
                    weighted_avg_review_for_claim = weighted_sum/sum(weights)
                    diff = abs(weighted_avg_review_for_claim - claim.author_review.value)
...
# deprecated, see BasedOnAvgDifferenceOfClaimsAndReviewsWithWeight
